        ...
        <table class="min-w-full bg-white border border-gray-300 rounded-lg">
          <thead class="bg-gray-200 text-gray-700">
            <tr>
              ...
              <th class="px-4 py-2 border">Código 2FA</th>
              <th class="px-4 py-2 border">Expiración</th>
          ...
          <tbody>
            <tr v-for="session in sessions" :key="session.id" class="hover:bg-gray-100 transition">
              <td class="px-4 py-2 border">{{ session.user?.name }}</td>
              <td class="px-4 py-2 border truncate max-w-xs">{{ session.token }}</td>
